...
5. **Add or remove detections if necessary.** Manual redaction boxes may be added to the plan using the schema in `references/plan-schema.md`. When a compliance policy applies (FERPA, HIPAA), load `references/entity-policy.md` and the matching profile (`references/ferpa.md` or `references/hipaa.md`) before editing the plan.
...
Default deterministic recognizers cover common structured identifiers such as email addresses, US SSNs, phone-like numbers (incl. fax), credit-card candidates with Luhn validation, IP addresses, URLs, ZIP codes, VINs, date-of-birth labels, and context-anchored unique ID numbers (medical record, student ID, account, beneficiary, license/certificate, serial). Optional Microsoft Presidio integration can add named entities such as people and locations when installed locally.

Use `--policy ferpa` or `--policy hipaa` to restrict a plan to the identifiers a particular framework protects. These filter the plan you review; they do not disable detectors. See `references/entity-policy.md`, `references/ferpa.md`, and `references/hipaa.md`.

**Native-text documents.** `scan-text` detects FERPA/HIPAA identifiers in the native text layer of a PDF without OCR. Use it alongside `scan` for mixed documents whose sensitive fields are real text (as is common in investigative reports and health records), then merge and review both plans.

Do not claim a named person was detected unless the recognizer actually returned that span. Do not invent sensitive values. ZIP, generic date, and ID-number recognizers over-flag by design — treat them as review candidates, not verdicts.
